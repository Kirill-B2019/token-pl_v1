    ...
    <div class="space-y-6 p-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($packages as $package)
                <div class="relative rounded-lg border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                    <div class="flex items-center justify-between">
                        <div>
                        ...
                        @if($package->discount_percentage)
                            <span class="relative z-10 rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-800 dark:bg-green-900/40 dark:text-green-200">-{{ $package->discount_percentage }}%</span>
                        @endif
                    </div>
                    ...
                    <a href="{{ route('client.packages.purchase', $package) }}" class="relative z-10 mt-4 inline-flex w-full justify-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-500 hover:shadow-md transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Купить</a>
                </div>
            @empty
        ...
        <x-admin.panel title="Доступные токены">
            <div class="grid gap-4 md:grid-cols-3">
                @forelse($tokens as $token)
                    <div class="relative rounded-md border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
