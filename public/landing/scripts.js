/* Copied from based_view/templatemo-nexus-scripts.js */
...
document.querySelectorAll('.btn-primary, .btn-secondary').forEach(button => {
    button.addEventListener('mouseenter', function() { this.style.boxShadow = '0 0 30px rgba(0, 255, 255, 0.6)'; });
    button.addEventListener('mouseleave', function() { this.style.boxShadow = ''; });
});
...
const animateStats = () => {
    ...
    stats.forEach(stat => {
        ...
        const timer = setInterval(() => {
            count += increment;
            if (count >= target) { clearInterval(timer); count = target; }
...
setInterval(() => {
    ...
    setTimeout(() => { document.body.removeChild(tempElement); }, 3000);
}, 5000);
